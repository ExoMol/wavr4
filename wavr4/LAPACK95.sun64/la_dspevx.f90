! this is MODIFIED lapack95 subroutine
! (the original is provided as la_dspevx.f90.org
! The problem was it did not do what it was intended to do.
! Strangely enough the line
!
!   ELSE IF( PRESENT(Z) .AND. ( S1Z /= LD .OR. S2Z /= N ) )THEN; LINFO = -4
!
! prohibited using the 2nd size of Z other than N thus forcing to have big
! size of Z even if a small number of eigenvectors is requested
! TO FIX THIS I SIMPLY REMOVED            .OR. S2Z /= N
!
! Also
!            W(LM+1:N) = 0.0_WP
! is replaced by
!            if(LM+1 <= N) W(LM+1:N) = 0.0_WP
! because if the boundaries are tested during the run time the program
! stops if LM+1 > N
!
! 23/07/02 I.K.

SUBROUTINE LA_SPEVX( A, W, UPLO, Z, VL, VU, IL, IU, &
                       M, IFAIL, ABSTOL, INFO )
!
...
!  .. USE STATEMENTS ..
   ...
   ! USE F77_LAPACK, ONLY: DSPEVX
   ! USE SUNPERF, ONLY: DSPEVX
!  .. IMPLICIT STATEMENT ..
   IMPLICIT NONE
!  .. CHARACTER ARGUMENTS ..
...
!  .. LOCAL SCALARS ..
   INTEGER :: LD, ISTAT, SIFAIL, S2Z, NN
   INTEGER(8) :: N, LINFO, LIL, LIU, LM, S1Z
   INTEGER(8), TARGET :: itmp1(1)
   INTEGER(8), TARGET :: ISTAT1(1)
   REAL(WP), TARGET :: LLZ(1,1)
   REAL(WP) :: LABSTOL, LVL, LVU
   COMPLEX(WP) :: WW
!  .. LOCAL ARRAYS ..
   INTEGER(8), POINTER :: IWORK(:), LIFAIL(:)
...
!   ELSE IF( PRESENT(Z) .AND. ( S1Z /= LD .OR. S2Z /= N ) )THEN; LINFO = -4
   ELSE IF( PRESENT(Z) .AND. ( S1Z /= LD ) )THEN; LINFO = -4
...
! the line below is commented out because IFAIL requires the type change
!             LIFAIL => IFAIL
             LIFAIL => itmp1
         ELSE; ALLOCATE( LIFAIL(N), STAT=ISTAT ); END IF
      ELSE; LJOBZ = 'N'; LIFAIL => ISTAT1; ENDIF
!     .. DETERMINE THE WORKSPACE
!   print *, ' determ workspace:'
      IF( ISTAT == 0 ) THEN
         ALLOCATE(IWORK(MAX(1,5*N)), WORK(MAX(1,8*N)), STAT=ISTAT)
         IF( ISTAT == 0 )THEN
            IF( PRESENT(ABSTOL) )THEN; LABSTOL = ABSTOL
            ELSE; LABSTOL = 2*DLAMCH('Safe minimum'); ENDIF
	    IF (PRESENT (Z)) THEN
               CALL DSPEVX_64( LJOBZ, LRANGE, LUPLO, N, A, LVL, LVU, &
