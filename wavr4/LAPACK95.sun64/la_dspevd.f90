SUBROUTINE LA_SPEVD( A, W, UPLO, Z, INFO )
!
...
!  .. USE STATEMENTS ..
   ...
   ! USE F77_LAPACK, ONLY: DSPEVD
   ! USE SUNPERF, ONLY: DSPEVD
!  .. IMPLICIT STATEMENT ..
   IMPLICIT NONE
!  .. CHARACTER ARGUMENTS ..
...
!  .. LOCAL SCALARS ..
   ...
   INTEGER :: NN, LD, ISTAT, ISTAT1, S1Z, itmp
   INTEGER(8) :: N, LINFO, S2Z, LWORK, LIWORK
   INTEGER, SAVE :: LWORKN = 0, LIWORKN = 0, LWORKV = 0, LIWORKV = 0
   COMPLEX(WP) :: WW
!  .. LOCAL ARRAYS ..
   REAL(WP), TARGET :: LLZ(1,1)
   INTEGER(8), POINTER :: IWORK(:)
...
!  .. DETERMINE THE WORKSPACE
      ...
      IF( ISTAT == 0 ) THEN
         IF( PRESENT(Z) )THEN
           CALL DSPEVD_64( LJOBZ, LUPLO, N, A, W, Z, S2Z, WORK, LWORK, &
                         IWORK, LIWORK, LINFO )
	 ELSE
  	   CALL DSPEVD_64( LJOBZ, LUPLO, N, A, W, LLZ, S2Z, WORK, LWORK, &
                         IWORK, LIWORK, LINFO )
	 ENDIF
         IF (LINFO == 0 ) THEN
            IF (LSAME(LJOBZ,'N')) THEN
