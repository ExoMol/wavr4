SUBROUTINE LA_SYEVX( A, W, JOBZ, UPLO, VL, VU, IL, IU, &
                       M, IFAIL, ABSTOL, INFO )
!
...
!  .. USE STATEMENTS ..
   ...
   ! USE F77_LAPACK, ONLY: DSYEVX
   ! USE F77_LAPACK, ONLY: ILAENV
   ! USE SUNPERF, ONLY: DSYEVX, ILAENV
!  .. IMPLICIT STATEMENT ..
   IMPLICIT NONE
!  .. CHARACTER ARGUMENTS ..
...
!  .. LOCAL SCALARS ..
   ...
   INTEGER :: LZ, NB, ISTAT, SIFAIL, itmp
   INTEGER(8) :: N, LINFO, LD, LDZ, LIL, LIU, LM, LWORK, itmp2, ILAENV_64
...
!  .. TEST THE ARGUMENTS
   ...
   ELSE IF( N > 0 )THEN
      ...
      ENDIF
      IF( PRESENT(IFAIL) )THEN;
! temporary fix because IFAIL is INT*4
         ! LIFAIL => IFAIL
         LIFAIL => itmp1
      ELSE
         LIFAIL => ISTAT1
...
!     .. DETERMINE THE WORKSPACE
      itmp2 = ILAENV_64( 1_8, BSNAME, LUPLO, N, -1_8, -1_8, -1_8 )
      NB = itmp2
...
!     .. CALL LAPACK77 ROUTINE
         CALL DSYEVX_64( LJOBZ, LRANGE, LUPLO, N, A, LD, LVL, LVU, &
