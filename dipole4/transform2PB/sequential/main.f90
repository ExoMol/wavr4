...
REAL(DP) :: zpe, tmp,temp,tmp1,temp1, energy, x, step, gmf
...
if(HHSYM) then
      ...
      ! constant factor after acting by P(HH) symmetry on angular basis
      gmf = one
      if (jp > 0) gmf = -one
      ! WARNING:  if K> 0 then gmf = (-1)**K
          ...
          if(ib > ic) then
            ...
            h5 => vectrs2(ib,ic)%mat(:,:)
              ...
              do im=1,idx2sym(ib,ic)%size
                ...
                tmp1 = vector3(n)
                if(ib /= ic) tmp1 = tmp1* one_over_sqrt2
                if(ib > ic)  tmp1 = tmp1* gmf
      ...
      ! allocate a vector
      allocate(vector3(istage2))

      read(u13) nstates,itmp

      if(nstates > 0)  open(u10,file=fname//'.dat', &
        access='SEQUENTIAL',form='UNFORMATTED',iostat=idiag)
