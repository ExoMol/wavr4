...
!      DOUBLE PRECISION A1, A1S, A2, A2S, C1, C1OLD, C2, CNORM, D1MACH,
     ...
     +                 DENOM, DV, EPS, XHUGE, L1, M1, NEWFAC, OLDFAC,
...
!      HUGE = SQRT(D1MACH(2)/20.0D0)
      XHUGE = SQRT(HUGE(1.0D0)/20.0D0)
!  i took it from a program which returned the value of HUGE for Alpha
!  however it was too big (1.797693134862316D+308),
!  so this one is a bit smaller
!      XHUGE = SQRT(1.79769313486231D+308/20.0D0)
      SRHUGE = SQRT(XHUGE)
      TINY = 1.0D0/XHUGE
      SRTINY = 1.0D0/SRHUGE
C
...
C  Check error condition 5.
   21 IER = 5
         write(6,'(2f10.5,2i5)') l1min,l1max,ndim,nfin
         print *, '  ier=', ier
